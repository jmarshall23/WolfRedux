...
qboolean G_ParseSpawnVars( void ) {
	...
	if ( !engine->trap_GetEntityToken( com_token, sizeof( com_token ) ) ) {
		// end of spawn string
		return qfalse;
	}
	if ( com_token[0] != '{' ) {
	...
	while ( 1 ) {
		// parse key
		if ( !engine->trap_GetEntityToken( keyname, sizeof( keyname ) ) ) {
...
void SP_worldspawn( void ) {
	...
	engine->trap_SetConfigstring( CS_GAME_VERSION, GAME_VERSION );

	engine->trap_SetConfigstring( CS_LEVEL_START_TIME, va( "%i", level.startTime ) );

	G_SpawnString( "music", "", &s );
	engine->trap_SetConfigstring( CS_MUSIC, s );

	G_SpawnString( "message", "", &s );
	engine->trap_SetConfigstring( CS_MESSAGE, s );              // map specific message

	engine->trap_SetConfigstring( CS_MOTD, g_motd.string );     // message of the day

	G_SpawnString( "gravity", "800", &s );
	engine->trap_Cvar_Set( "g_gravity", s );
	...
	engine->trap_SetConfigstring( CS_WARMUP, "" );
	if ( g_restarted.integer ) {
		engine->trap_Cvar_Set( "g_restarted", "0" );
		level.warmupTime = 0;
	}
