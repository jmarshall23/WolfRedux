...
void AssetCache() {
	...
	//}
	//Assets.background = engine->trap_R_RegisterShaderNoMip( ASSET_BACKGROUND );
	//Com_Printf("Menu Size: %i bytes\n", sizeof(Menus));
	uiInfo.uiDC.Assets.gradientBar = engine->trap_R_RegisterShaderNoMip( ASSET_GRADIENTBAR );
	uiInfo.uiDC.Assets.fxBasePic = engine->trap_R_RegisterShaderNoMip( ART_FX_BASE );
	uiInfo.uiDC.Assets.fxPic[0] = engine->trap_R_RegisterShaderNoMip( ART_FX_RED );
	uiInfo.uiDC.Assets.fxPic[1] = engine->trap_R_RegisterShaderNoMip( ART_FX_YELLOW );
	uiInfo.uiDC.Assets.fxPic[2] = engine->trap_R_RegisterShaderNoMip( ART_FX_GREEN );
	uiInfo.uiDC.Assets.fxPic[3] = engine->trap_R_RegisterShaderNoMip( ART_FX_TEAL );
	uiInfo.uiDC.Assets.fxPic[4] = engine->trap_R_RegisterShaderNoMip( ART_FX_BLUE );
	uiInfo.uiDC.Assets.fxPic[5] = engine->trap_R_RegisterShaderNoMip( ART_FX_CYAN );
	uiInfo.uiDC.Assets.fxPic[6] = engine->trap_R_RegisterShaderNoMip( ART_FX_WHITE );
	uiInfo.uiDC.Assets.scrollBar = engine->trap_R_RegisterShaderNoMip( ASSET_SCROLLBAR );
	uiInfo.uiDC.Assets.scrollBarArrowDown = engine->trap_R_RegisterShaderNoMip( ASSET_SCROLLBAR_ARROWDOWN );
	uiInfo.uiDC.Assets.scrollBarArrowUp = engine->trap_R_RegisterShaderNoMip( ASSET_SCROLLBAR_ARROWUP );
	uiInfo.uiDC.Assets.scrollBarArrowLeft = engine->trap_R_RegisterShaderNoMip( ASSET_SCROLLBAR_ARROWLEFT );
	uiInfo.uiDC.Assets.scrollBarArrowRight = engine->trap_R_RegisterShaderNoMip( ASSET_SCROLLBAR_ARROWRIGHT );
	uiInfo.uiDC.Assets.scrollBarThumb = engine->trap_R_RegisterShaderNoMip( ASSET_SCROLL_THUMB );
	uiInfo.uiDC.Assets.sliderBar = engine->trap_R_RegisterShaderNoMip( ASSET_SLIDER_BAR );
	uiInfo.uiDC.Assets.sliderThumb = engine->trap_R_RegisterShaderNoMip( ASSET_SLIDER_THUMB );

	for ( n = 0; n < NUM_CROSSHAIRS; n++ ) {
		uiInfo.uiDC.Assets.crosshairShader[n] = engine->trap_R_RegisterShaderNoMip( va( "gfx/2d/crosshair%c", 'a' + n ) );
	}

//	uiInfo.newHighScoreSound = engine->trap_S_RegisterSound("sound/feedback/voc_newhighscore.wav");
}

void _UI_DrawSides( float x, float y, float w, float h, float size ) {
	UI_AdjustFrom640( &x, &y, &w, &h );
	size *= uiInfo.uiDC.xscale;
	engine->trap_R_DrawStretchPic( x, y, size, h, 0, 0, 0, 0, uiInfo.uiDC.whiteShader );
	engine->trap_R_DrawStretchPic( x + w - size, y, size, h, 0, 0, 0, 0, uiInfo.uiDC.whiteShader );
}

void _UI_DrawTopBottom( float x, float y, float w, float h, float size ) {
	UI_AdjustFrom640( &x, &y, &w, &h );
	size *= uiInfo.uiDC.yscale;
	engine->trap_R_DrawStretchPic( x, y, w, size, 0, 0, 0, 0, uiInfo.uiDC.whiteShader );
	engine->trap_R_DrawStretchPic( x, y + h - size, w, size, 0, 0, 0, 0, uiInfo.uiDC.whiteShader );
}
/*
================
UI_DrawRect
...
void UI_ShowPostGame( qboolean newHigh ) {
	engine->trap_Cvar_Set( "cg_cameraOrbit", "0" );
	engine->trap_Cvar_Set( "cg_thirdPerson", "0" );
	engine->trap_Cvar_Set( "sv_killserver", "1" );
...
void _UI_Refresh( int realtime ) {
	...
	//if ( !( engine->trap_Key_GetCatcher() & KEYCATCH_UI ) ) {
	//	return;
	//}
...
void _UI_Shutdown( void ) {
	engine->trap_LAN_SaveCachedServers();
}
...
char *GetMenuBuffer( const char *filename ) {
	...
	if ( !f ) {
		engine->trap_Print( va( S_COLOR_RED "menu file not found: %s, using default\n", filename ) );
		return defaultMenu;
	}
	if ( len >= MAX_MENUFILE ) {
		engine->trap_Print( va( S_COLOR_RED "menu file too large: %s is %i, max allowed is %i", filename, len, MAX_MENUFILE ) );
		engine->trap_FS_FCloseFile( f );
		return defaultMenu;
	}

	engine->trap_FS_Read( buf, len, f );
	buf[len] = 0;
	engine->trap_FS_FCloseFile( f );
	//COM_Compress(buf);
	return buf;
...
qboolean Asset_Parse( int handle ) {
	...
	while ( 1 ) {
		...
		if ( Q_stricmp( token.string, "font" ) == 0 ) {
			...
			engine->trap_R_RegisterFont( tempStr, pointSize, &uiInfo.uiDC.Assets.textFont );
		...
		if ( Q_stricmp( token.string, "smallFont" ) == 0 ) {
			...
			engine->trap_R_RegisterFont( tempStr, pointSize, &uiInfo.uiDC.Assets.smallFont );
			continue;
		}
		...
		if ( Q_stricmp( token.string, "bigFont" ) == 0 ) {
			...
			engine->trap_R_RegisterFont( tempStr, pointSize, &uiInfo.uiDC.Assets.bigFont );
			continue;
		}
		...
		if ( Q_stricmp( token.string, "handwritingFont" ) == 0 ) {
			...
			engine->trap_R_RegisterFont( tempStr, pointSize, &uiInfo.uiDC.Assets.handwritingFont );
			continue;
		}
		...
		if ( Q_stricmp( token.string, "gradientbar" ) == 0 ) {
			...
			uiInfo.uiDC.Assets.gradientBar = engine->trap_R_RegisterShaderNoMip( tempStr );
			continue;
		}
		...
		if ( Q_stricmp( token.string, "cursor" ) == 0 ) {
			...
			uiInfo.uiDC.Assets.cursor = engine->trap_R_RegisterShaderNoMip( uiInfo.uiDC.Assets.cursorStr );
			continue;
		}
...
void UI_ParseMenu( const char *menuFile ) {
	...
	handle = engine->trap_PC_LoadSource( menuFile );
	if ( !handle ) {
		return;
	}
	...
	engine->trap_PC_FreeSource( handle );
}
...
void UI_LoadMenus( const char *menuFile, qboolean reset ) {
	...
	start = engine->trap_Milliseconds();

	handle = engine->trap_PC_LoadSource( menuFile );
	if ( !handle ) {
		engine->trap_Error( va( S_COLOR_YELLOW "menu file not found: %s, using default\n", menuFile ) );
#ifdef WOLF_SP_DEMO
		handle = engine->trap_PC_LoadSource( "ui/menus.txt" );
#else
		handle = engine->trap_PC_LoadSource( "ui/demomenus.txt" );
#endif
		if ( !handle ) {
			engine->trap_Error( va( S_COLOR_RED "default menu file not found: ui/menus.txt, unable to continue!\n", menuFile ) );
		}
	}
	...
	Com_Printf( "UI menu load time = %d milli seconds\n", engine->trap_Milliseconds() - start );
...
static void UI_DrawHandicap( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	...
	h = Com_Clamp( 5, 100, engine->trap_Cvar_VariableValue( "handicap" ) );
	i = 20 - h / 5;
...
static void UI_SetCapFragLimits( qboolean uiVars ) {
	...
	if ( uiVars ) {
		engine->trap_Cvar_Set( "ui_captureLimit", va( "%d", cap ) );
		engine->trap_Cvar_Set( "ui_fragLimit", va( "%d", frag ) );
	} else {
		engine->trap_Cvar_Set( "capturelimit", va( "%d", cap ) );
		engine->trap_Cvar_Set( "fraglimit", va( "%d", frag ) );
	}
}
...
static void UI_DrawNetGameType( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	if ( ui_netGameType.integer < 0 || ui_netGameType.integer > uiInfo.numGameTypes ) {
		engine->trap_Cvar_Set( "ui_netGameType", "0" );
		engine->trap_Cvar_Set( "ui_actualNetGameType", "0" );
	}
...
static void UI_DrawJoinGameType( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	if ( ui_joinGameType.integer < 0 || ui_joinGameType.integer > uiInfo.numJoinGameTypes ) {
		engine->trap_Cvar_Set( "ui_joinGameType", "0" );
	}
...
static void UI_DrawSaveGameShot( rectDef_t *rect, float scale, vec4_t color ) {
	...
	if ( !strlen( ui_savegameName.string ) || ui_savegameName.string[0] == '0' ) {
		image = engine->trap_R_RegisterShaderNoMip( "menu/art/unknownmap" );
	} else {
		i = UI_SavegameIndexFromName2( ui_savegameName.string );
//	mapName
		if ( uiInfo.savegameList[i].sshotImage == -1 ) {
			uiInfo.savegameList[i].sshotImage = engine->trap_R_RegisterShaderNoMip( uiInfo.savegameList[i].imageName );
		}
...
static void UI_DrawClanLogo( rectDef_t *rect, float scale, vec4_t color ) {
	...
	if ( i >= 0 && i < uiInfo.teamCount ) {
		...
		if ( uiInfo.teamList[i].teamIcon == -1 ) {
			uiInfo.teamList[i].teamIcon         = engine->trap_R_RegisterShaderNoMip( uiInfo.teamList[i].imageName );
			uiInfo.teamList[i].teamIcon_Metal   = engine->trap_R_RegisterShaderNoMip( va( "%s_metal",uiInfo.teamList[i].imageName ) );
			uiInfo.teamList[i].teamIcon_Name    = engine->trap_R_RegisterShaderNoMip( va( "%s_name", uiInfo.teamList[i].imageName ) );
		}
	...
	if ( i >= 0 && i < uiInfo.teamCount ) {
		...
		if ( uiInfo.teamList[i].cinematic >= -2 ) {
			if ( uiInfo.teamList[i].cinematic == -1 ) {
				uiInfo.teamList[i].cinematic = engine->trap_CIN_PlayCinematic( va( "%s.roq", uiInfo.teamList[i].imageName ), 0, 0, 0, 0, ( CIN_loop | CIN_silent ) );
			}
			if ( uiInfo.teamList[i].cinematic >= 0 ) {
				engine->trap_CIN_RunCinematic( uiInfo.teamList[i].cinematic );
				engine->trap_CIN_SetExtents( uiInfo.teamList[i].cinematic, rect->x, rect->y, rect->w, rect->h );
				engine->trap_CIN_DrawCinematic( uiInfo.teamList[i].cinematic );
			} else {
...
static void UI_DrawPregameCinematic( rectDef_t *rect, float scale, vec4_t color ) {
	if ( uiInfo.previewMovie > -2 ) {
		uiInfo.previewMovie = engine->trap_CIN_PlayCinematic( va( "%s.roq", "assault" ), 0, 0, 0, 0, ( CIN_loop | CIN_silent | CIN_system ) );
		if ( uiInfo.previewMovie >= 0 ) {
			engine->trap_CIN_RunCinematic( uiInfo.previewMovie );
			engine->trap_CIN_SetExtents( uiInfo.previewMovie, rect->x, rect->y, rect->w, rect->h );
			engine->trap_CIN_DrawCinematic( uiInfo.previewMovie );
		} else {
...
static void UI_DrawPreviewCinematic( rectDef_t *rect, float scale, vec4_t color ) {
	if ( uiInfo.previewMovie > -2 ) {
		uiInfo.previewMovie = engine->trap_CIN_PlayCinematic( va( "%s.roq", uiInfo.movieList[uiInfo.movieIndex] ), 0, 0, 0, 0, ( CIN_loop | CIN_silent ) );
		if ( uiInfo.previewMovie >= 0 ) {
			engine->trap_CIN_RunCinematic( uiInfo.previewMovie );
			engine->trap_CIN_SetExtents( uiInfo.previewMovie, rect->x, rect->y, rect->w, rect->h );
			engine->trap_CIN_DrawCinematic( uiInfo.previewMovie );
...
static void UI_DrawSkill( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	int i;
	i = engine->trap_Cvar_VariableValue( "g_spSkill" );
...
static void UI_DrawTeamMember( rectDef_t *rect, int font, float scale, vec4_t color, qboolean blue, int num, int textStyle ) {
	...
	// 1 - Human
	// 2..NumCharacters - Bot
	int value = engine->trap_Cvar_VariableValue( va( blue ? "ui_blueteam%i" : "ui_redteam%i", num ) );
	const char *text;
	if ( value <= 0 ) {
		text = "Closed";
	} else if ( value == 1 ) {
...
static void UI_DrawMapLevelshot( rectDef_t *rect ) {
	...
	if ( levelname && levelname[0] != 0 ) {
		levelshot = engine->trap_R_RegisterShaderNoMip( va( "levelshots/%s.tga", levelname ) );
	}
...
static void UI_DrawMapPreview( rectDef_t *rect, float scale, vec4_t color, qboolean net ) {
	...
	if ( map < 0 || map > uiInfo.mapCount ) {
		if ( net ) {
			...
			engine->trap_Cvar_Set( "ui_currentNetMap", "0" );
		} else {
			ui_currentMap.integer = 0;
			engine->trap_Cvar_Set( "ui_currentMap", "0" );
		}
		map = 0;
	}

	if ( uiInfo.mapList[map].levelShot == -1 ) {
		uiInfo.mapList[map].levelShot = engine->trap_R_RegisterShaderNoMip( uiInfo.mapList[map].imageName );
	}

	if ( uiInfo.mapList[map].levelShot > 0 ) {
		UI_DrawHandlePic( rect->x, rect->y, rect->w, rect->h, uiInfo.mapList[map].levelShot );
	} else {
		UI_DrawHandlePic( rect->x, rect->y, rect->w, rect->h, engine->trap_R_RegisterShaderNoMip( "menu/art/unknownmap" ) );
	}
}
...
static void UI_DrawMapCinematic( rectDef_t *rect, float scale, vec4_t color, qboolean net ) {
	...
	if ( uiInfo.mapList[map].cinematic >= -1 ) {
		if ( uiInfo.mapList[map].cinematic == -1 ) {
			uiInfo.mapList[map].cinematic = engine->trap_CIN_PlayCinematic( va( "%s.roq", uiInfo.mapList[map].mapLoadName ), 0, 0, 0, 0, ( CIN_loop | CIN_silent ) );
		}
		if ( uiInfo.mapList[map].cinematic >= 0 ) {
			engine->trap_CIN_RunCinematic( uiInfo.mapList[map].cinematic );
			engine->trap_CIN_SetExtents( uiInfo.mapList[map].cinematic, rect->x, rect->y, rect->w, rect->h );
			engine->trap_CIN_DrawCinematic( uiInfo.mapList[map].cinematic );
		} else {
...
static void UI_DrawPlayerModel( rectDef_t *rect ) {
	...
	if ( engine->trap_Cvar_VariableValue( "ui_Q3Model" ) ) {
	...
	// compare new cvars to old cvars and see if we need to update
	{
		...
		v1 = engine->trap_Cvar_VariableValue( "mp_team" );
		v2 = engine->trap_Cvar_VariableValue( "ui_prevTeam" );
		if ( v1 != v2 ) {
			engine->trap_Cvar_Set( "ui_prevTeam", va( "%i", v1 ) );
			updateModel = qtrue;
		}

		v1 = engine->trap_Cvar_VariableValue( "mp_playerType" );
		v2 = engine->trap_Cvar_VariableValue( "ui_prevClass" );
		if ( v1 != v2 ) {
			engine->trap_Cvar_Set( "ui_prevClass", va( "%i", v1 ) );
			updateModel = qtrue;
		}

		v1 = engine->trap_Cvar_VariableValue( "mp_weapon" );
		v2 = engine->trap_Cvar_VariableValue( "ui_prevWeapon" );
		if ( v1 != v2 ) {
			engine->trap_Cvar_Set( "ui_prevWeapon", va( "%i", v1 ) );
			updateModel = qtrue;
		}
	}
...
static void UI_DrawNetMapCinematic( rectDef_t *rect, float scale, vec4_t color ) {
	...
	if ( uiInfo.serverStatus.currentServerCinematic >= 0 ) {
		engine->trap_CIN_RunCinematic( uiInfo.serverStatus.currentServerCinematic );
		engine->trap_CIN_SetExtents( uiInfo.serverStatus.currentServerCinematic, rect->x, rect->y, rect->w, rect->h );
		engine->trap_CIN_DrawCinematic( uiInfo.serverStatus.currentServerCinematic );
	} else {
...
static void UI_DrawTier( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	int i;
	i = engine->trap_Cvar_VariableValue( "ui_currentTier" );
...
static void UI_DrawTierMap( rectDef_t *rect, int index ) {
	...
	if ( uiInfo.tierList[i].mapHandles[index] == -1 ) {
		uiInfo.tierList[i].mapHandles[index] = engine->trap_R_RegisterShaderNoMip( va( "levelshots/%s", uiInfo.tierList[i].maps[index] ) );
	}
...
static void UI_DrawTierMapName( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	...
	j = engine->trap_Cvar_VariableValue( "ui_currentMap" );
...
static void UI_NextOpponent() {
	...
	engine->trap_Cvar_Set( "ui_opponentName", uiInfo.teamList[i].teamName );
}
...
//----(SA)	end
	...
	case UI_BLUETEAM4:
	case UI_BLUETEAM5:
		value = engine->trap_Cvar_VariableValue( va( "ui_blueteam%i", ownerDraw - UI_BLUETEAM1 + 1 ) );
		if ( value <= 0 ) {
			text = "Closed";
		} else if ( value == 1 ) {
			text = "Human";
	...
	case UI_REDTEAM4:
	case UI_REDTEAM5:
		value = engine->trap_Cvar_VariableValue( va( "ui_redteam%i", ownerDraw - UI_REDTEAM1 + 1 ) );
		if ( value <= 0 ) {
			text = "Closed";
		} else if ( value == 1 ) {
			text = "Human";
...
static void UI_DrawBotName( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	...
	int game = engine->trap_Cvar_VariableValue( "g_gametype" );
	const char *text = "";
	if ( game >= GT_TEAM ) {
		if ( value >= uiInfo.characterCount ) {
			value = 0;
...
static void UI_BuildPlayerList() {
	...
	engine->trap_GetClientState( &cs );
	engine->trap_GetConfigString( CS_PLAYERS + cs.clientNum, info, MAX_INFO_STRING );
	uiInfo.playerNumber = cs.clientNum;
	uiInfo.teamLeader = atoi( Info_ValueForKey( info, "tl" ) );
	team = atoi( Info_ValueForKey( info, "t" ) );
	engine->trap_GetConfigString( CS_SERVERINFO, info, sizeof( info ) );
	count = atoi( Info_ValueForKey( info, "sv_maxclients" ) );
	uiInfo.playerCount = 0;
	uiInfo.myTeamCount = 0;
	playerTeamNumber = 0;
	for ( n = 0; n < count; n++ ) {
		engine->trap_GetConfigString( CS_PLAYERS + n, info, MAX_INFO_STRING );
	...
	if ( !uiInfo.teamLeader ) {
		engine->trap_Cvar_Set( "cg_selectedPlayer", va( "%d", playerTeamNumber ) );
	}

	n = engine->trap_Cvar_VariableValue( "cg_selectedPlayer" );
	if ( n < 0 || n > uiInfo.myTeamCount ) {
		n = 0;
	}
	if ( n < uiInfo.myTeamCount ) {
		engine->trap_Cvar_Set( "cg_selectedPlayerName", uiInfo.teamNames[n] );
	}
}
...
static void UI_DrawServerRefreshDate( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	...
	if ( uiInfo.serverStatus.refreshActive ) {
		...
		Text_Paint( rect->x, rect->y, font, scale, newColor, va( "Getting info for %d servers (ESC to cancel)", engine->trap_LAN_GetServerCount( ui_netSource.integer ) ), 0, 0, textStyle );
	} else {
		char buff[64];
...
static void UI_DrawLimboChat( rectDef_t *rect, int font, float scale, vec4_t color, int textStyle ) {
	...
	for ( i = 0, count = 0; ; i++, count++ ) {
		if ( !engine->trap_GetLimboString( i, buf ) ) {
			break;
		}
	}
	...
	for ( i = 0; i < count; i++ ) {
		...
		engine->trap_GetLimboString( i, buf );
...
static qboolean UI_OwnerDrawVisible( int flags ) {
	...
	while ( flags ) {
		...
		if ( flags & UI_SHOW_FFA ) {
			if ( engine->trap_Cvar_VariableValue( "g_gametype" ) != GT_FFA ) {
				vis = qfalse;
			}
			flags &= ~UI_SHOW_FFA;
		}
		...
		if ( flags & UI_SHOW_NEWHIGHSCORE ) {
			if ( uiInfo.newHighScoreTime < uiInfo.uiDC.realTime ) {
				...
				if ( uiInfo.soundHighScore ) {
					if ( engine->trap_Cvar_VariableValue( "sv_killserver" ) == 0 ) {
						// wait on server to go down before playing sound
						engine->trap_S_StartLocalSound( uiInfo.newHighScoreSound, CHAN_ANNOUNCER );
...
static qboolean UI_Handicap_HandleKey( int flags, float *special, int key ) {
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		engine->trap_Cvar_Set( "handicap", va( "%i", h ) );
		return qtrue;
	}
	return qfalse;
}
...
static qboolean UI_Effects_HandleKey( int flags, float *special, int key ) {
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		engine->trap_Cvar_SetValue( "color", uitogamecode[uiInfo.effectsColor] );
		return qtrue;
	}
	return qfalse;
}
...
static qboolean UI_SavegameName_HandleKey( int flags, float *special, int key ) {
	...
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		engine->trap_Cvar_Set( "ui_savegameName", uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[i]].savegameName );
		engine->trap_Cvar_Set( "ui_savegameInfo", uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[i]].savegameInfoText );
		return qtrue;
	}
	return qfalse;
}
...
static qboolean UI_ClanName_HandleKey( int flags, float *special, int key ) {
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		i = UI_TeamIndexFromName( UI_Cvar_VariableString( "ui_teamName" ) );
		if ( uiInfo.teamList[i].cinematic >= 0 ) {
			engine->trap_CIN_StopCinematic( uiInfo.teamList[i].cinematic );
		...
		engine->trap_Cvar_Set( "ui_teamName", uiInfo.teamList[i].teamName );
		updateModel = qtrue;
		return qtrue;
	}
	return qfalse;
...
static qboolean UI_GameType_HandleKey( int flags, float *special, int key, qboolean resetMap ) {
	...
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		if ( uiInfo.gameTypes[ui_gameType.integer].gtEnum == GT_TOURNAMENT ) {
			engine->trap_Cvar_Set( "ui_Q3Model", "1" );
		} else {
			engine->trap_Cvar_Set( "ui_Q3Model", "0" );
		}

		engine->trap_Cvar_Set( "ui_gameType", va( "%d", ui_gameType.integer ) );
...
static qboolean UI_NetGameType_HandleKey( int flags, float *special, int key ) {
	...
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		engine->trap_Cvar_Set( "ui_netGameType", va( "%d", ui_netGameType.integer ) );
		engine->trap_Cvar_Set( "ui_actualnetGameType", va( "%d", uiInfo.gameTypes[ui_netGameType.integer].gtEnum ) );
		engine->trap_Cvar_Set( "ui_currentNetMap", "0" );
...
static qboolean UI_JoinGameType_HandleKey( int flags, float *special, int key ) {
	...
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		engine->trap_Cvar_Set( "ui_joinGameType", va( "%d", ui_joinGameType.integer ) );
...
static qboolean UI_Skill_HandleKey( int flags, float *special, int key ) {
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		int i = engine->trap_Cvar_VariableValue( "g_spSkill" );
		...
		engine->trap_Cvar_Set( "g_spSkill", va( "%i", i ) );
		return qtrue;
	}
	return qfalse;
}
...
static qboolean UI_TeamName_HandleKey( int flags, float *special, int key, qboolean blue ) {
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		engine->trap_Cvar_Set( ( blue ) ? "ui_blueTeam" : "ui_redTeam", uiInfo.teamList[i].teamName );
...
static qboolean UI_TeamMember_HandleKey( int flags, float *special, int key, qboolean blue, int num ) {
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		int value = engine->trap_Cvar_VariableValue( cvar );
		...
		engine->trap_Cvar_Set( cvar, va( "%i", value ) );
		return qtrue;
	}
	return qfalse;
}
...
static qboolean UI_NetSource_HandleKey( int flags, float *special, int key ) {
	...
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		engine->trap_Cvar_Set( "ui_netSource", va( "%d", ui_netSource.integer ) );
		return qtrue;
	}
#endif  // #ifdef MISSIONPACK
	return qfalse;
...
static qboolean UI_Crosshair_HandleKey( int flags, float *special, int key ) {
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		engine->trap_Cvar_Set( "cg_drawCrosshair", va( "%d", uiInfo.currentCrosshair ) );
		return qtrue;
	}
	return qfalse;
}
...
static qboolean UI_SelectedPlayer_HandleKey( int flags, float *special, int key ) {
	if ( key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER ) {
		...
		selected = engine->trap_Cvar_VariableValue( "cg_selectedPlayer" );
		...
		if ( selected == uiInfo.myTeamCount ) {
			...
			engine->trap_Cvar_Set( "cg_selectedPlayerName", uiInfo.teamNames[selected] );
		}
		engine->trap_Cvar_Set( "cg_selectedPlayer", va( "%d", selected ) );
	}
	return qfalse;
}
...
static int QDECL UI_ServersQsortCompare( const void *arg1, const void *arg2 ) {
#ifdef MISSIONPACK
	return engine->trap_LAN_CompareServers( ui_netSource.integer, uiInfo.serverStatus.sortKey, uiInfo.serverStatus.sortDir, *(int*)arg1, *(int*)arg2 );
#else
	return qfalse;
#endif  // #ifdef MISSIONPACK
}
...
void UI_SavegameSort( int column, qboolean force ) {
	...
	if ( uiInfo.savegameCount ) {
		...
		engine->trap_Cvar_Set( "ui_savegame", "" );

	} else {
		engine->trap_Cvar_Set( "ui_savegameName", "" );
		engine->trap_Cvar_Set( "ui_savegameInfo", "(no savegames)" );
	}
...
static void UI_LoadMods() {
	...
	uiInfo.modCount = 0;
	numdirs = engine->trap_FS_GetFileList( "$modlist", "", dirlist, sizeof( dirlist ) );
	dirptr  = dirlist;
	for ( i = 0; i < numdirs; i++ ) {
...
static void UI_LoadTeams() {
	...
	count = engine->trap_FS_GetFileList( "", "team", teamList, 4096 );
...
static void UI_DelSavegame() {
	...
	ret = engine->trap_FS_Delete( va( "save/%s.svg", uiInfo.savegameList[i].savegameFile ) );
...
void UI_ParseSavegame( int index ) {
	...
	engine->trap_FS_FOpenFile( va( "save/%s.svg", uiInfo.savegameList[index].savegameFile ), &f, FS_READ );
	if ( !f ) {
		return;
	}

	// read the version
	engine->trap_FS_Read( &ver, sizeof( i ), f );
	...
	if ( ver > 14 ) {
		...
		engine->trap_FS_Read( &tm->tm_sec, sizeof( tm->tm_sec ), f );          // secs after the min
		engine->trap_FS_Read( &tm->tm_min, sizeof( tm->tm_min ), f );          // mins after the hour
		engine->trap_FS_Read( &tm->tm_hour, sizeof( tm->tm_hour ), f );        // hrs since midnight
		engine->trap_FS_Read( &tm->tm_mday, sizeof( tm->tm_mday ), f );
		engine->trap_FS_Read( &tm->tm_mon, sizeof( tm->tm_mon ), f );
		engine->trap_FS_Read( &tm->tm_year, sizeof( tm->tm_year ), f );        // yrs from 1900
		engine->trap_FS_Read( &tm->tm_wday, sizeof( tm->tm_wday ), f );
		engine->trap_FS_Read( &tm->tm_yday, sizeof( tm->tm_yday ), f );        // days since jan1 (0-365)
...
static void UI_LoadSavegames( char *dir ) {
	...
	if ( dir ) {
		uiInfo.savegameCount = engine->trap_FS_GetFileList( va( "save/%s", dir ), "svg", sglist, 4096 );
	} else {
		uiInfo.savegameCount = engine->trap_FS_GetFileList( "save", "svg", sglist, 4096 );
	}
...
//			qsort( &uiInfo.savegameStatus.displaySavegames[0], uiInfo.savegameCount, sizeof(int), UI_SavegamesQsortCompare);
			...
			if ( uiInfo.savegameList[i].episode != -1 ) {
				uiInfo.savegameList[i].sshotImage = engine->trap_R_RegisterShaderNoMip( va( "levelshots/episodeshots/e%d.tga", uiInfo.savegameList[i].episode + 1 ) );
			} else {
				uiInfo.savegameList[i].sshotImage = engine->trap_R_RegisterShaderNoMip( "levelshots/episodeshots/e_unknown.tga" );
			}
...
static void UI_LoadMovies() {
	...
	uiInfo.movieCount = engine->trap_FS_GetFileList( "video", "roq", movielist, 4096 );
...
static void UI_LoadDemos() {
	...
	Com_sprintf( demoExt, sizeof( demoExt ), "dm_%d", (int)engine->trap_Cvar_VariableValue( "protocol" ) );

	uiInfo.demoCount = engine->trap_FS_GetFileList( "demos", demoExt, demolist, 4096 );
...
static void UI_StartSkirmish( qboolean next ) {
	...
	if ( next ) {
		int actual;
		int index = engine->trap_Cvar_VariableValue( "ui_mapIndex" );
	...
	engine->trap_Cvar_SetValue( "g_gametype", g );
	engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( "wait ; wait ; map %s\n", uiInfo.mapList[ui_currentMap.integer].mapLoadName ) );
	skill = engine->trap_Cvar_VariableValue( "g_spSkill" );
	engine->trap_Cvar_Set( "ui_scoreMap", uiInfo.mapList[ui_currentMap.integer].mapName );

	k = UI_TeamIndexFromName( UI_Cvar_VariableString( "ui_opponentName" ) );

	engine->trap_Cvar_Set( "ui_singlePlayerActive", "1" );

	// set up sp overrides, will be replaced on postgame
	temp = engine->trap_Cvar_VariableValue( "capturelimit" );
	engine->trap_Cvar_Set( "ui_saveCaptureLimit", va( "%i", temp ) );
	temp = engine->trap_Cvar_VariableValue( "fraglimit" );
	engine->trap_Cvar_Set( "ui_saveFragLimit", va( "%i", temp ) );

	UI_SetCapFragLimits( qfalse );

	temp = engine->trap_Cvar_VariableValue( "cg_drawTimer" );
	engine->trap_Cvar_Set( "ui_drawTimer", va( "%i", temp ) );
	temp = engine->trap_Cvar_VariableValue( "g_doWarmup" );
	engine->trap_Cvar_Set( "ui_doWarmup", va( "%i", temp ) );
	temp = engine->trap_Cvar_VariableValue( "g_friendlyFire" );
	engine->trap_Cvar_Set( "ui_friendlyFire", va( "%i", temp ) );
	temp = engine->trap_Cvar_VariableValue( "sv_maxClients" );
	engine->trap_Cvar_Set( "ui_maxClients", va( "%i", temp ) );
	temp = engine->trap_Cvar_VariableValue( "g_warmup" );
	engine->trap_Cvar_Set( "ui_Warmup", va( "%i", temp ) );
	temp = engine->trap_Cvar_VariableValue( "sv_pure" );
	engine->trap_Cvar_Set( "ui_pure", va( "%i", temp ) );

	engine->trap_Cvar_Set( "cg_cameraOrbit", "0" );
	engine->trap_Cvar_Set( "cg_thirdPerson", "0" );
	engine->trap_Cvar_Set( "cg_drawTimer", "1" );
	engine->trap_Cvar_Set( "g_doWarmup", "1" );
	engine->trap_Cvar_Set( "g_warmup", "15" );
	engine->trap_Cvar_Set( "sv_pure", "0" );
	engine->trap_Cvar_Set( "g_friendlyFire", "0" );
	engine->trap_Cvar_Set( "g_redTeam", UI_Cvar_VariableString( "ui_teamName" ) );
	engine->trap_Cvar_Set( "g_blueTeam", UI_Cvar_VariableString( "ui_opponentName" ) );

	if ( engine->trap_Cvar_VariableValue( "ui_recordSPDemo" ) ) {
		Com_sprintf( buff, MAX_STRING_CHARS, "%s_%i", uiInfo.mapList[ui_currentMap.integer].mapLoadName, g );
		engine->trap_Cvar_Set( "ui_recordSPDemoName", buff );
	}

	delay = 500;

	if ( g == GT_TOURNAMENT ) {
		engine->trap_Cvar_Set( "sv_maxClients", "2" );
		Com_sprintf( buff, sizeof( buff ), "wait ; addbot %s %f " ", %i \n", uiInfo.mapList[ui_currentMap.integer].opponentName, skill, delay );
		engine->trap_Cmd_ExecuteText( EXEC_APPEND, buff );
	} else {
		temp = uiInfo.mapList[ui_currentMap.integer].teamMembers * 2;
		engine->trap_Cvar_Set( "sv_maxClients", va( "%d", temp ) );
	...
	if ( g >= GT_TEAM ) {
		engine->trap_Cmd_ExecuteText( EXEC_APPEND, "wait 5; team Red\n" );
	}
#endif  // #ifdef MISSIONPACK
}
...
int WM_getWeaponIndex() {
	...
	lookupIndex = engine->trap_Cvar_VariableValue( "mp_weapon" );
...
static void WM_ChangePlayerType() {
	...
	playerType = engine->trap_Cvar_VariableValue( "mp_playerType" );
	...
	for ( i = 0; i < 4; i++ ) {
		...
		if ( i == playerType ) {
			Menu_ShowItemByName((menuDef_t*)itemdef->parent, playerTypes[i].name, qtrue );
		} else {
			Menu_ShowItemByName((menuDef_t*)itemdef->parent, playerTypes[i].name, qfalse );
		}
...
void WM_GetSpawnPoints() {
	...
	for ( i = 1; i < uiInfo.spawnCount; i++ ) {
		engine->trap_GetConfigString( CS_MULTI_SPAWNTARGETS + i, cs, sizeof( cs ) );
...
void WM_PickItem( int selectionType, int itemIndex ) {
	...
	if ( selectionType == WM_SELECT_TEAM ) {
		switch ( itemIndex ) {
		case WM_AXIS:
			engine->trap_Cvar_Set( "mp_team", "0" );
			break;
		case WM_ALLIES:
			engine->trap_Cvar_Set( "mp_team", "1" );
			break;
		case WM_SPECTATOR:
			engine->trap_Cvar_Set( "mp_team", "2" );
			break;
		}
		...
		case WM_START_SELECT:
			break;
		case WM_SOLDIER:
			engine->trap_Cvar_Set( "mp_playerType", "0" );
			engine->trap_Cvar_Set( "mp_weapon", "0" );
			break;
		case WM_MEDIC:
			engine->trap_Cvar_Set( "mp_playerType", "1" );
			engine->trap_Cvar_Set( "mp_weapon", "0" );
			break;
		case WM_ENGINEER:
			engine->trap_Cvar_Set( "mp_playerType", "2" );
			engine->trap_Cvar_Set( "mp_weapon", "0" );
			break;
		case WM_LIEUTENANT:
			engine->trap_Cvar_Set( "mp_playerType", "3" );
			engine->trap_Cvar_Set( "mp_weapon", "0" );
			break;
		}
		...
		if ( itemIndex == WM_START_SELECT ) {
			...
			engine->trap_Cvar_Set( weaponTypes[itemIndex].cvar, va( "%i", weaponTypes[itemIndex].value ) );
		}
...
void WM_LimboChat() {
	...
	engine->trap_Cvar_VariableStringBuffer( "ui_cmd", buf, 200 );

	if ( strlen( buf ) ) {
		engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( "say_limbo %s\n", buf ) );
	}

	engine->trap_Cvar_Set( "ui_cmd", "" );
}
...
static void UI_Update( const char *name ) {
	int val = engine->trap_Cvar_VariableValue( name );

	if ( Q_stricmp( name, "ui_SetName" ) == 0 ) {
		engine->trap_Cvar_Set( "name", UI_Cvar_VariableString( "ui_Name" ) );
	} else if ( Q_stricmp( name, "ui_setRate" ) == 0 ) {
		float rate = engine->trap_Cvar_VariableValue( "rate" );
		if ( rate >= 5000 ) {
			engine->trap_Cvar_Set( "cl_maxpackets", "30" );
			engine->trap_Cvar_Set( "cl_packetdup", "1" );
		} else if ( rate >= 4000 ) {
			engine->trap_Cvar_Set( "cl_maxpackets", "15" );
			engine->trap_Cvar_Set( "cl_packetdup", "2" );       // favor less prediction errors when there's packet loss
		} else {
			engine->trap_Cvar_Set( "cl_maxpackets", "15" );
			engine->trap_Cvar_Set( "cl_packetdup", "1" );       // favor lower bandwidth
		}
	} else if ( Q_stricmp( name, "ui_GetName" ) == 0 ) {
		engine->trap_Cvar_Set( "ui_Name", UI_Cvar_VariableString( "name" ) );
	} else if ( Q_stricmp( name, "r_colorbits" ) == 0 ) {
		switch ( val ) {
		case 0:
			engine->trap_Cvar_SetValue( "r_depthbits", 0 );
			engine->trap_Cvar_SetValue( "r_stencilbits", 0 );
			break;
		case 16:
			engine->trap_Cvar_SetValue( "r_depthbits", 16 );
			engine->trap_Cvar_SetValue( "r_stencilbits", 0 );
			break;
		case 32:
			engine->trap_Cvar_SetValue( "r_depthbits", 24 );
			break;
		}
	} else if ( Q_stricmp( name, "r_lodbias" ) == 0 ) {
		switch ( val ) {
		case 0:
			engine->trap_Cvar_SetValue( "r_subdivisions", 4 );
			break;
		case 1:
			engine->trap_Cvar_SetValue( "r_subdivisions", 12 );
			break;
		case 2:
			engine->trap_Cvar_SetValue( "r_subdivisions", 20 );
			break;
		}
	} else if ( Q_stricmp( name, "ui_glCustom" ) == 0 ) {
		switch ( val ) {
		case 0: // high quality
			engine->trap_Cvar_SetValue( "r_fullScreen", 1 );
			engine->trap_Cvar_SetValue( "r_subdivisions", 4 );
			engine->trap_Cvar_SetValue( "r_vertexlight", 0 );
			engine->trap_Cvar_SetValue( "r_lodbias", 0 );
			engine->trap_Cvar_SetValue( "r_colorbits", 32 );
			engine->trap_Cvar_SetValue( "r_depthbits", 24 );
			engine->trap_Cvar_SetValue( "r_picmip", 0 );
			engine->trap_Cvar_SetValue( "r_picmip2", 0 );
			engine->trap_Cvar_SetValue( "r_mode", 4 );
			engine->trap_Cvar_SetValue( "r_texturebits", 32 );
			engine->trap_Cvar_SetValue( "r_fastSky", 0 );
			engine->trap_Cvar_SetValue( "r_inGameVideo", 1 );
			engine->trap_Cvar_SetValue( "cg_shadows", 1 );
			engine->trap_Cvar_SetValue( "cg_brassTime", 2500 );
			engine->trap_Cvar_Set( "r_texturemode", "GL_LINEAR_MIPMAP_LINEAR" );
			break;
		case 1: // normal
			engine->trap_Cvar_SetValue( "r_fullScreen", 1 );
			engine->trap_Cvar_SetValue( "r_subdivisions", 12 );
			engine->trap_Cvar_SetValue( "r_vertexlight", 0 );
			engine->trap_Cvar_SetValue( "r_lodbias", 0 );
			engine->trap_Cvar_SetValue( "r_colorbits", 0 );
			engine->trap_Cvar_SetValue( "r_depthbits", 24 );
			engine->trap_Cvar_SetValue( "r_picmip", 0 );
			engine->trap_Cvar_SetValue( "r_picmip2", 1 );
			engine->trap_Cvar_SetValue( "r_mode", 3 );
			engine->trap_Cvar_SetValue( "r_texturebits", 0 );
			engine->trap_Cvar_SetValue( "r_fastSky", 0 );
			engine->trap_Cvar_SetValue( "r_inGameVideo", 1 );
			engine->trap_Cvar_SetValue( "cg_brassTime", 2500 );
			engine->trap_Cvar_Set( "r_texturemode", "GL_LINEAR_MIPMAP_NEAREST" ); //----(SA)	modified so wolf never sets trilinear automatically
			engine->trap_Cvar_SetValue( "cg_shadows", 0 );
			break;
		case 2: // fast
			engine->trap_Cvar_SetValue( "r_fullScreen", 1 );
			engine->trap_Cvar_SetValue( "r_subdivisions", 8 );
			engine->trap_Cvar_SetValue( "r_vertexlight", 0 );
			engine->trap_Cvar_SetValue( "r_lodbias", 1 );
			engine->trap_Cvar_SetValue( "r_colorbits", 0 );
			engine->trap_Cvar_SetValue( "r_depthbits", 0 );
			engine->trap_Cvar_SetValue( "r_picmip", 1 );
			engine->trap_Cvar_SetValue( "r_picmip2", 2 );
			engine->trap_Cvar_SetValue( "r_mode", 3 );
			engine->trap_Cvar_SetValue( "r_texturebits", 0 );
			engine->trap_Cvar_SetValue( "cg_shadows", 0 );
			engine->trap_Cvar_SetValue( "r_fastSky", 1 );
			engine->trap_Cvar_SetValue( "r_inGameVideo", 0 );
			engine->trap_Cvar_SetValue( "cg_brassTime", 0 );
			engine->trap_Cvar_Set( "r_texturemode", "GL_LINEAR_MIPMAP_NEAREST" );
			break;
		case 3: // fastest
			engine->trap_Cvar_SetValue( "r_fullScreen", 1 );
			engine->trap_Cvar_SetValue( "r_subdivisions", 20 );
			engine->trap_Cvar_SetValue( "r_vertexlight", 1 );
			engine->trap_Cvar_SetValue( "r_lodbias", 2 );
			engine->trap_Cvar_SetValue( "r_colorbits", 16 );
			engine->trap_Cvar_SetValue( "r_depthbits", 16 );
			engine->trap_Cvar_SetValue( "r_mode", 3 );
			engine->trap_Cvar_SetValue( "r_picmip", 2 );
			engine->trap_Cvar_SetValue( "r_picmip2", 3 );
			engine->trap_Cvar_SetValue( "r_texturebits", 16 );
			engine->trap_Cvar_SetValue( "cg_shadows", 0 );
			engine->trap_Cvar_SetValue( "cg_brassTime", 0 );
			engine->trap_Cvar_SetValue( "r_fastSky", 1 );
			engine->trap_Cvar_SetValue( "r_inGameVideo", 0 );
		...
		if ( val == 0 ) {
			engine->trap_Cvar_SetValue( "m_pitch", 0.022f );
		} else {
			engine->trap_Cvar_SetValue( "m_pitch", -0.022f );
		}
...
static void UI_RunMenuScript( char **args ) {
	...
	if ( String_Parse( args, &name ) ) {
		...
		if ( Q_stricmp( name, "StartServer" ) == 0 ) {
			...
			engine->trap_Cvar_Set( "cg_thirdPerson", "0" );
			engine->trap_Cvar_Set( "cg_cameraOrbit", "0" );
			engine->trap_Cvar_Set( "ui_singlePlayerActive", "0" );
			engine->trap_Cvar_SetValue( "dedicated", Com_Clamp( 0, 2, ui_dedicated.integer ) );
			engine->trap_Cvar_SetValue( "g_gametype", Com_Clamp( 0, 8, uiInfo.gameTypes[ui_netGameType.integer].gtEnum ) );
			engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( "wait ; wait ; map %s\n", uiInfo.mapList[ui_currentNetMap.integer].mapLoadName ) );
			skill = engine->trap_Cvar_VariableValue( "g_spSkill" );
			// set max clients based on spots
			clients = 0;
			for ( i = 0; i < PLAYERS_PER_TEAM; i++ ) {
				int bot = engine->trap_Cvar_VariableValue( va( "ui_blueteam%i", i + 1 ) );
				if ( bot >= 0 ) {
					clients++;
				}
				bot = engine->trap_Cvar_VariableValue( va( "ui_redteam%i", i + 1 ) );
				if ( bot >= 0 ) {
					clients++;
				}
			}
			...
			engine->trap_Cvar_Set( "sv_maxClients", va( "%d",clients ) );
			...
			engine->trap_Cvar_Set( "ui_mapIndex", va( "%d", ui_mapIndex.integer ) );
			Menu_SetFeederSelection( NULL, FEEDER_MAPS, ui_mapIndex.integer, "skirmish" );
			UI_GameType_HandleKey( 0, 0, K_MOUSE1, qfalse );
			UI_GameType_HandleKey( 0, 0, K_MOUSE2, qfalse );
		} else if ( Q_stricmp( name, "resetDefaults" ) == 0 ) {
// SP was like...
//			engine->trap_Cmd_ExecuteText( EXEC_APPEND, "exec default.cfg\n");
//			engine->trap_Cmd_ExecuteText( EXEC_APPEND, "cvar_restart\n");
//			Controls_SetDefaults();
//			engine->trap_Cvar_Set("com_introPlayed", "1" );
//			engine->trap_Cmd_ExecuteText( EXEC_APPEND, "vid_restart\n" );

// from MP 11/12/01
			engine->trap_Cmd_ExecuteText( EXEC_NOW, "cvar_restart\n" );            // NERVE - SMF - changed order
			engine->trap_Cmd_ExecuteText( EXEC_NOW, "exec default.cfg\n" );
			engine->trap_Cmd_ExecuteText( EXEC_NOW, "exec language.cfg\n" );       // NERVE - SMF
			engine->trap_Cmd_ExecuteText( EXEC_NOW, "setRecommended\n" );     // NERVE - SMF
			Controls_SetDefaults();
			engine->trap_Cvar_Set( "com_introPlayed", "1" );
			engine->trap_Cvar_Set( "com_recommendedSet", "1" );                   // NERVE - SMF
			engine->trap_Cmd_ExecuteText( EXEC_APPEND, "vid_restart\n" );
// end from MP
		} else if ( Q_stricmp( name, "getCDKey" ) == 0 ) {
			char out[17];
			engine->trap_GetCDKey( buff, 17 );
			engine->trap_Cvar_Set( "cdkey1", "" );
			engine->trap_Cvar_Set( "cdkey2", "" );
			engine->trap_Cvar_Set( "cdkey3", "" );
			engine->trap_Cvar_Set( "cdkey4", "" );
			if ( strlen( buff ) == CDKEY_LEN ) {
				Q_strncpyz( out, buff, 5 );
				engine->trap_Cvar_Set( "cdkey1", out );
				Q_strncpyz( out, buff + 4, 5 );
				engine->trap_Cvar_Set( "cdkey2", out );
				Q_strncpyz( out, buff + 8, 5 );
				engine->trap_Cvar_Set( "cdkey3", out );
				Q_strncpyz( out, buff + 12, 5 );
				engine->trap_Cvar_Set( "cdkey4", out );
			}
			...
			engine->trap_Cvar_Set( "cdkey", buff );
			if ( engine->trap_VerifyCDKey( buff, UI_Cvar_VariableString( "cdkeychecksum" ) ) ) {
				engine->trap_Cvar_Set( "ui_cdkeyvalid", "CD Key Appears to be valid." );
				engine->trap_SetCDKey( buff );
			} else {
				engine->trap_Cvar_Set( "ui_cdkeyvalid", "CD Key does not appear to be valid." );
			}
			//#ifdef MISSIONPACK			// NERVE - SMF - enabled for multiplayer
			...
			engine->trap_Cvar_Set( "com_errorMessage", "" );
			//#ifdef MISSIONPACK			// NERVE - SMF - enabled for multiplayer
			...
			if ( uiInfo.demoAvailable ) {
				engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( "demo %s_%i", uiInfo.mapList[ui_currentMap.integer].mapLoadName, uiInfo.gameTypes[ui_gameType.integer].gtEnum ) );
			}
			//#endif	// #ifdef MISSIONPACK`
			...
			if ( DC->getCVarValue( "developer" ) ) {
				...
				engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( "loadgame %s\n", uiInfo.savegameList[i].savegameFile ) );
			}
			...
			if ( !strlen( name ) ) {
				...
				if ( i == uiInfo.savegameCount ) {
					engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( "savegame %s\n", UI_Cvar_VariableString( "ui_savegame" ), MAX_NAME_LENGTH ) );
					Menus_CloseAll();
				}
			}
			...
			engine->trap_Cmd_ExecuteText( EXEC_APPEND, "fade 0 0 0 0 3\n" );    // fade screen up
			engine->trap_Cvar_Set( "g_playerstart", "1" );                 // set cvar which will trigger "playerstart" in script
			Menus_CloseAll();
			//----(SA)	end
			...
			if ( uiInfo.previewMovie >= 0 ) {
				engine->trap_CIN_StopCinematic( uiInfo.previewMovie );
			}
			engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( "cinematic %s.roq 2\n", uiInfo.movieList[uiInfo.movieIndex] ) );
		} else if ( Q_stricmp( name, "RunMod" ) == 0 ) {
			engine->trap_Cvar_Set( "fs_game", uiInfo.modList[uiInfo.modIndex].modName );
			engine->trap_Cmd_ExecuteText( EXEC_APPEND, "vid_restart;" );
		} else if ( Q_stricmp( name, "RunDemo" ) == 0 ) {
			engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( "demo %s\n", uiInfo.demoList[uiInfo.demoIndex] ) );
		} else if ( Q_stricmp( name, "Wolf" ) == 0 ) {
			engine->trap_Cvar_Set( "fs_game", "" );
			engine->trap_Cmd_ExecuteText( EXEC_APPEND, "vid_restart;" );
			//#ifdef MISSIONPACK			// NERVE - SMF - enabled for multiplayer
			...
			engine->trap_LAN_GetServerAddressString( ui_netSource.integer, uiInfo.serverStatus.displayServers[uiInfo.serverStatus.currentServer], uiInfo.serverStatusAddress, sizeof( uiInfo.serverStatusAddress ) );
			...
			if ( uiInfo.serverStatus.currentServer >= 0 && uiInfo.serverStatus.currentServer < uiInfo.serverStatus.numDisplayServers ) {
				engine->trap_LAN_GetServerAddressString( ui_netSource.integer, uiInfo.serverStatus.displayServers[uiInfo.serverStatus.currentServer], buff, 1024 );
				engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( "connect %s\n", buff ) );
			}
			...
			engine->trap_Cvar_Set( "ui_singlePlayerActive", "0" );
			engine->trap_Cmd_ExecuteText( EXEC_NOW, "quit" );
		} else if ( Q_stricmp( name, "Controls" ) == 0 ) {
			engine->trap_Cvar_Set( "cl_paused", "1" );
			engine->trap_Key_SetCatcher( KEYCATCH_UI );
			Menus_CloseAll();
			Menus_ActivateByName( "setup_menu2" );
		} else if ( Q_stricmp( name, "Leave" ) == 0 ) {
			engine->trap_Cmd_ExecuteText( EXEC_APPEND, "disconnect\n" );
			engine->trap_Key_SetCatcher( KEYCATCH_UI );
			Menus_CloseAll();
			...
			engine->trap_Key_SetCatcher( engine->trap_Key_GetCatcher() & ~KEYCATCH_UI );
			engine->trap_Key_ClearStates();
			engine->trap_Cvar_Set( "cl_paused", "0" );
			Menus_CloseAll();
			//#ifdef MISSIONPACK			// NERVE - SMF - enabled for multiplayer
		} else if ( Q_stricmp( name, "voteMap" ) == 0 ) {
			if ( ui_currentNetMap.integer >= 0 && ui_currentNetMap.integer < uiInfo.mapCount ) {
				engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( "callvote map %s\n",uiInfo.mapList[ui_currentNetMap.integer].mapLoadName ) );
			}
			//#endif	// #ifdef MISSIONPACK
		} else if ( Q_stricmp( name, "voteKick" ) == 0 ) {
			if ( uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount ) {
				engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( "callvote kick %s\n",uiInfo.playerNames[uiInfo.playerIndex] ) );
			}
		} else if ( Q_stricmp( name, "voteGame" ) == 0 ) {
			if ( ui_netGameType.integer >= 0 && ui_netGameType.integer < uiInfo.numGameTypes ) {
				engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( "callvote g_gametype %i\n",uiInfo.gameTypes[ui_netGameType.integer].gtEnum ) );
			}
		} else if ( Q_stricmp( name, "voteLeader" ) == 0 ) {
			if ( uiInfo.teamIndex >= 0 && uiInfo.teamIndex < uiInfo.myTeamCount ) {
				engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( "callteamvote leader %s\n",uiInfo.teamNames[uiInfo.teamIndex] ) );
			}
			//#ifdef MISSIONPACK			// NERVE - SMF - enabled for multiplayer
		} else if ( Q_stricmp( name, "addBot" ) == 0 ) {
			if ( engine->trap_Cvar_VariableValue( "g_gametype" ) >= GT_TEAM ) {
				engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( "addbot %s %i %s\n", uiInfo.characterList[uiInfo.botIndex].name, uiInfo.skillIndex + 1, ( uiInfo.redBlue == 0 ) ? "Red" : "Blue" ) );
			} else {
				// NERVE - SMF - no bots in wolf multiplayer
			...
			if ( ui_netSource.integer != AS_FAVORITES ) {
				...
				engine->trap_LAN_GetServerInfo( ui_netSource.integer, uiInfo.serverStatus.displayServers[uiInfo.serverStatus.currentServer], buff, MAX_STRING_CHARS );
				name[0] = addr[0] = '\0';
				Q_strncpyz( name,    Info_ValueForKey( buff, "hostname" ), MAX_NAME_LENGTH );
				Q_strncpyz( addr,    Info_ValueForKey( buff, "addr" ), MAX_NAME_LENGTH );
				if ( strlen( name ) > 0 && strlen( addr ) > 0 ) {
					res = engine->trap_LAN_AddServer( AS_FAVORITES, name, addr );
					if ( res == 0 ) {
			...
			if ( ui_netSource.integer == AS_FAVORITES ) {
				...
				Q_strncpyz( addr,    Info_ValueForKey( buff, "addr" ), MAX_NAME_LENGTH );
				if ( strlen( addr ) > 0 ) {
					engine->trap_LAN_RemoveServer( AS_FAVORITES, addr );
				}
			}
			...
			if ( String_Parse( args, &orders ) ) {
				int selectedPlayer = engine->trap_Cvar_VariableValue( "cg_selectedPlayer" );
				if ( selectedPlayer < uiInfo.myTeamCount ) {
					strcpy( buff, orders );
					engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( buff, uiInfo.teamClientNums[selectedPlayer] ) );
					engine->trap_Cmd_ExecuteText( EXEC_APPEND, "\n" );
				} else {
					int i;
					for ( i = 0; i < uiInfo.myTeamCount; i++ ) {
						...
						strcpy( buff, orders );
						engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( buff, uiInfo.teamNames[i] ) );
						engine->trap_Cmd_ExecuteText( EXEC_APPEND, "\n" );
					}
				}
				engine->trap_Key_SetCatcher( engine->trap_Key_GetCatcher() & ~KEYCATCH_UI );
			...
			if ( String_Parse( args, &orders ) ) {
				int selectedPlayer = engine->trap_Cvar_VariableValue( "cg_selectedPlayer" );
				if ( selectedPlayer == uiInfo.myTeamCount ) {
					engine->trap_Cmd_ExecuteText( EXEC_APPEND, orders );
					engine->trap_Cmd_ExecuteText( EXEC_APPEND, "\n" );
				}
				engine->trap_Key_SetCatcher( engine->trap_Key_GetCatcher() & ~KEYCATCH_UI );
				engine->trap_Key_ClearStates();
			...
			engine->trap_Cvar_Set( "ui_glCustom", "4" );
...
//----(SA)	// start other .exe
			...
			engine->trap_Cmd_ExecuteText( EXEC_APPEND, "startMultiplayer\n" );
...
//----(SA)
			...
			// get cvars
			team = engine->trap_Cvar_VariableValue( "mp_team" );
			playerType = engine->trap_Cvar_VariableValue( "mp_playerType" );
			weapon = engine->trap_Cvar_VariableValue( "mp_weapon" );
			pistol = engine->trap_Cvar_VariableValue( "mp_pistol" );
			item1 = engine->trap_Cvar_VariableValue( "mp_item1" );
			...
			if ( team != 2 ) {
				engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( "limbomessage %s %s %s\n", teamStr, classStr, weapStr ) );
			}

			// join team
			if ( team == 0 ) {
				engine->trap_Cmd_ExecuteText( EXEC_APPEND, va( "team %s %i %i %i %i 1\n", "red", playerType, weapon, pistol, item1 ) );
			} else if ( team == 1 ) {
			...
			engine->trap_Cmd_ExecuteText( EXEC_APPEND, "setRecommended 1\n" );
		} else {
...
static void UI_BinaryServerInsertion( int num ) {
	...
	while ( mid > 0 ) {
		mid = len >> 1;
		//
		res = engine->trap_LAN_CompareServers( ui_netSource.integer, uiInfo.serverStatus.sortKey,
...
static void UI_BuildServerDisplayList( qboolean force ) {
	...
	engine->trap_Cvar_VariableStringBuffer( "cl_motdString", uiInfo.serverStatus.motd, sizeof( uiInfo.serverStatus.motd ) );
	...
	if ( force ) {
		...
		engine->trap_LAN_MarkServerVisible( ui_netSource.integer, -1, qtrue );
	}

	// get the server count (comes from the master)
	count = engine->trap_LAN_GetServerCount( ui_netSource.integer );
	...
	for ( i = 0; i < count; i++ ) {
		...
		if ( !engine->trap_LAN_ServerIsVisible( ui_netSource.integer, i ) ) {
			continue;
		}
		visible = qtrue;
		// get the ping for this server
		ping = engine->trap_LAN_GetServerPing( ui_netSource.integer, i );
		if ( ping > 0 || ui_netSource.integer == AS_FAVORITES ) {

			engine->trap_LAN_GetServerInfo( ui_netSource.integer, i, info, MAX_STRING_CHARS );
			...
			if ( ui_browserShowEmpty.integer == 0 ) {
				if ( clients == 0 ) {
					engine->trap_LAN_MarkServerVisible( ui_netSource.integer, i, qfalse );
					continue;
				}
			}
...
static void UI_BuildFindPlayerList( qboolean force ) {
	...
	if ( !force ) {
		...
		engine->trap_Cvar_VariableStringBuffer( "ui_findPlayer", uiInfo.findPlayerName, sizeof( uiInfo.findPlayerName ) );
		...
		engine->trap_Cvar_Set( "cl_serverStatusResendTime", va( "%d", resend ) );
		// reset all server status requests
		engine->trap_LAN_ServerStatus( NULL, NULL, 0 );
		//
	...
	for ( i = 0; i < MAX_SERVERSTATUSREQUESTS; i++ ) {
		...
		if ( !uiInfo.pendingServerStatus.server[i].valid ||
			...
			if ( uiInfo.pendingServerStatus.num < uiInfo.serverStatus.numDisplayServers ) {
				...
				engine->trap_LAN_GetServerAddressString( ui_netSource.integer, uiInfo.serverStatus.displayServers[uiInfo.pendingServerStatus.num],
												 uiInfo.pendingServerStatus.server[i].adrstr, sizeof( uiInfo.pendingServerStatus.server[i].adrstr ) );
				engine->trap_LAN_GetServerInfo( ui_netSource.integer, uiInfo.serverStatus.displayServers[uiInfo.pendingServerStatus.num], infoString, sizeof( infoString ) );
...
static void UI_UpdatePendingPings() {
	//#ifdef MISSIONPACK			// NERVE - SMF - enabled for multiplayer
	engine->trap_LAN_ResetPings( ui_netSource.integer );
	...
	if ( feederID == FEEDER_HEADS ) {
		...
		if ( index >= 0 && index < uiInfo.serverStatus.numDisplayServers ) {
			...
			if ( lastServerColumn != column || lastServerTime > uiInfo.uiDC.realTime + 5000 ) {
				engine->trap_LAN_GetServerInfo( ui_netSource.integer, uiInfo.serverStatus.displayServers[index], info, MAX_STRING_CHARS );
	...
	if ( feederID == FEEDER_HEADS ) {
		if ( index >= 0 && index < uiInfo.characterCount ) {
			if ( uiInfo.characterList[index].headImage == -1 ) {
				uiInfo.characterList[index].headImage = engine->trap_R_RegisterShaderNoMip( uiInfo.characterList[index].imageName );
			}
		...
		if ( index >= 0 && index < uiInfo.mapCount ) {
			if ( uiInfo.mapList[index].levelShot == -1 ) {
				uiInfo.mapList[index].levelShot = engine->trap_R_RegisterShaderNoMip( uiInfo.mapList[index].imageName );
			}
		...
		if ( index >= 0 && index < uiInfo.savegameCount ) {
			if ( uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[index]].sshotImage == -1 ) {
				uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[index]].sshotImage = engine->trap_R_RegisterShaderNoMip( va( "levelshots/%s.tga",uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[index]].mapName ) );
			}
...
static void UI_FeederSelection( float feederID, int index ) {
	...
	if ( feederID == FEEDER_HEADS ) {
		if ( index >= 0 && index < uiInfo.characterCount ) {
			engine->trap_Cvar_Set( "team_model", uiInfo.characterList[index].female ? "janet" : "james" );
			engine->trap_Cvar_Set( "team_headmodel", va( "*%s", uiInfo.characterList[index].name ) );
			updateModel = qtrue;
		}
	} else if ( feederID == FEEDER_Q3HEADS ) {
		if ( index >= 0 && index < uiInfo.q3HeadCount ) {
			engine->trap_Cvar_Set( "model", uiInfo.q3HeadNames[index] );
			engine->trap_Cvar_Set( "headmodel", uiInfo.q3HeadNames[index] );
			updateModel = qtrue;
		}
		//#ifdef MISSIONPACK			// NERVE - SMF - enabled for multiplayer
	} else if ( feederID == FEEDER_MAPS || feederID == FEEDER_ALLMAPS ) {
		...
		if ( uiInfo.mapList[map].cinematic >= 0 ) {
			engine->trap_CIN_StopCinematic( uiInfo.mapList[map].cinematic );
			uiInfo.mapList[map].cinematic = -1;
		}
		UI_SelectedMap( index, &actual );
		engine->trap_Cvar_Set( "ui_mapIndex", va( "%d", index ) );
		ui_mapIndex.integer = index;

		if ( feederID == FEEDER_MAPS ) {
			ui_currentMap.integer = actual;
			engine->trap_Cvar_Set( "ui_currentMap", va( "%d", actual ) );
			uiInfo.mapList[ui_currentMap.integer].cinematic = engine->trap_CIN_PlayCinematic( va( "%s.roq", uiInfo.mapList[ui_currentMap.integer].mapLoadName ), 0, 0, 0, 0, ( CIN_loop | CIN_silent ) );
			UI_LoadBestScores( uiInfo.mapList[ui_currentMap.integer].mapLoadName, uiInfo.gameTypes[ui_gameType.integer].gtEnum );
			engine->trap_Cvar_Set( "ui_opponentModel", uiInfo.mapList[ui_currentMap.integer].opponentName );
			updateOpponentModel = qtrue;
		} else {
			ui_currentNetMap.integer = actual;
			engine->trap_Cvar_Set( "ui_currentNetMap", va( "%d", actual ) );
			uiInfo.mapList[ui_currentNetMap.integer].cinematic = engine->trap_CIN_PlayCinematic( va( "%s.roq", uiInfo.mapList[ui_currentNetMap.integer].mapLoadName ), 0, 0, 0, 0, ( CIN_loop | CIN_silent ) );
		}
		...
		engine->trap_LAN_GetServerInfo( ui_netSource.integer, uiInfo.serverStatus.displayServers[index], info, MAX_STRING_CHARS );
		uiInfo.serverStatus.currentServerPreview = engine->trap_R_RegisterShaderNoMip( va( "levelshots/%s", Info_ValueForKey( info, "mapname" ) ) );
		if ( uiInfo.serverStatus.currentServerCinematic >= 0 ) {
			engine->trap_CIN_StopCinematic( uiInfo.serverStatus.currentServerCinematic );
			uiInfo.serverStatus.currentServerCinematic = -1;
		}
		mapName = Info_ValueForKey( info, "mapname" );
		if ( mapName && *mapName ) {
			uiInfo.serverStatus.currentServerCinematic = engine->trap_CIN_PlayCinematic( va( "%s.roq", mapName ), 0, 0, 0, 0, ( CIN_loop | CIN_silent ) );
		}
	} else if ( feederID == FEEDER_SERVERSTATUS ) {
		//
	} else if ( feederID == FEEDER_FINDPLAYER ) {
...
//		itemDef_t *itemdef;
		...
		if ( index >= 0 && index < uiInfo.savegameCount ) {
			...
			// the text entry box
			engine->trap_Cvar_Set( "ui_savegame", uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[index]].savegameName );

			engine->trap_Cvar_Set( "ui_savegameName", uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[index]].savegameName );
			engine->trap_Cvar_Set( "ui_savegameInfo", uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[index]].savegameInfoText );
...
//			itemdef->cursorPos = 0;
		...
		engine->trap_Cmd_ExecuteText( EXEC_NOW, va( "setspawnpt %i\n", index ) );
	}
	// -NERVE - SMF
}
...
static qboolean Team_Parse(char **p) {
	...
	while ( 1 ) {
		...
		if (token[0] == '{') {
			...
			uiInfo.teamList[uiInfo.teamCount].teamIcon = engine->trap_R_RegisterShaderNoMip(uiInfo.teamList[uiInfo.teamCount].imageName);
			uiInfo.teamList[uiInfo.teamCount].teamIcon_Metal = engine->trap_R_RegisterShaderNoMip(va("%s_metal",uiInfo.teamList[uiInfo.teamCount].imageName));
			uiInfo.teamList[uiInfo.teamCount].teamIcon_Name = engine->trap_R_RegisterShaderNoMip(va("%s_name", uiInfo.teamList[uiInfo.teamCount].imageName));
...
static qboolean MapList_Parse( char **p ) {
	...
	while ( 1 ) {
		...
		if ( token[0] == '{' ) {
			...
			//  uiInfo.mapList[uiInfo.mapCount].cinematic = engine->trap_CIN_PlayCinematic(va("%s.roq",uiInfo.mapList[uiInfo.mapCount].mapLoadName), qfalse, qfalse, qtrue, 0, 0, 0, 0);
			//}
			uiInfo.mapList[uiInfo.mapCount].cinematic = -1;
			uiInfo.mapList[uiInfo.mapCount].levelShot = engine->trap_R_RegisterShaderNoMip( va( "levelshots/%s_small", uiInfo.mapList[uiInfo.mapCount].mapLoadName ) );
...
static int UI_PlayCinematic( const char *name, float x, float y, float w, float h ) {
	return engine->trap_CIN_PlayCinematic( name, x, y, w, h, ( CIN_loop | CIN_silent ) );
}
	...
	if ( handle >= 0 ) {
		...
		if ( handle == UI_MAPCINEMATIC ) {
			if ( uiInfo.mapList[ui_currentMap.integer].cinematic >= 0 ) {
				engine->trap_CIN_StopCinematic( uiInfo.mapList[ui_currentMap.integer].cinematic );
...
static void UI_BuildQ3Model_List( void ) {
	...
	numdirs = engine->trap_FS_GetFileList( "models/players", "/", dirlist, 2048 );
	dirptr  = dirlist;
	for ( i = 0; i < numdirs && uiInfo.q3HeadCount < MAX_PLAYERMODELS; i++,dirptr += dirlen + 1 )
	{
		...
		numfiles = engine->trap_FS_GetFileList( va( "models/players/%s",dirptr ), "tga", filelist, 2048 );
		fileptr  = filelist;
		for ( j = 0; j < numfiles && uiInfo.q3HeadCount < MAX_PLAYERMODELS; j++,fileptr += filelen + 1 )
		{
			...
			if ( Q_stricmpn( skinname, "icon_", 5 ) == 0 && !( Q_stricmp( skinname,"icon_blue" ) == 0 || Q_stricmp( skinname,"icon_red" ) == 0 ) ) {
				...
				uiInfo.q3HeadIcons[uiInfo.q3HeadCount++] = engine->trap_R_RegisterShaderNoMip( va( "models/players/%s/%s",dirptr,skinname ) );
			}
...
void _UI_Init( qboolean inGameLoad ) {
	...
	engine->trap_GetGlconfig( &uiInfo.uiDC.glconfig );
	...
	//UI_Load();
	uiInfo.uiDC.registerShaderNoMip = engine->trap_R_RegisterShaderNoMip;
	uiInfo.uiDC.setColor = &UI_SetColor;
	uiInfo.uiDC.drawHandlePic = &UI_DrawHandlePic;
	uiInfo.uiDC.drawStretchPic = engine->trap_R_DrawStretchPic;
	uiInfo.uiDC.drawText = &Text_Paint;
	uiInfo.uiDC.textWidth = &Text_Width;
	uiInfo.uiDC.textHeight = &Text_Height;
	uiInfo.uiDC.registerModel = engine->trap_R_RegisterModel;
	uiInfo.uiDC.modelBounds = engine->trap_R_ModelBounds;
	uiInfo.uiDC.fillRect = &UI_FillRect;
	uiInfo.uiDC.drawRect = &_UI_DrawRect;
	uiInfo.uiDC.drawSides = &_UI_DrawSides;
	uiInfo.uiDC.drawTopBottom = &_UI_DrawTopBottom;
	uiInfo.uiDC.clearScene = engine->trap_R_ClearScene;
	uiInfo.uiDC.drawSides = &_UI_DrawSides;
	uiInfo.uiDC.addRefEntityToScene = engine->trap_R_AddRefEntityToScene;
	uiInfo.uiDC.renderScene = engine->trap_R_RenderScene;
	uiInfo.uiDC.registerFont = engine->trap_R_RegisterFont;
	uiInfo.uiDC.ownerDrawItem = &UI_OwnerDraw;
	uiInfo.uiDC.getValue = &UI_GetValue;
	uiInfo.uiDC.ownerDrawVisible = &UI_OwnerDrawVisible;
	uiInfo.uiDC.runScript = &UI_RunMenuScript;
	uiInfo.uiDC.getTeamColor = &UI_GetTeamColor;
	uiInfo.uiDC.setCVar = engine->trap_Cvar_Set;
	uiInfo.uiDC.getCVarString = engine->trap_Cvar_VariableStringBuffer;
	uiInfo.uiDC.getCVarValue = engine->trap_Cvar_VariableValue;
	uiInfo.uiDC.drawTextWithCursor = &Text_PaintWithCursor;
	uiInfo.uiDC.setOverstrikeMode = engine->trap_Key_SetOverstrikeMode;
	uiInfo.uiDC.getOverstrikeMode = engine->trap_Key_GetOverstrikeMode;
	uiInfo.uiDC.startLocalSound = engine->trap_S_StartLocalSound;
	...
	uiInfo.uiDC.setBinding = engine->trap_Key_SetBinding;
	uiInfo.uiDC.getBindingBuf = engine->trap_Key_GetBindingBuf;
	uiInfo.uiDC.keynumToStringBuf = engine->trap_Key_KeynumToStringBuf;
	uiInfo.uiDC.executeText = engine->trap_Cmd_ExecuteText;
	uiInfo.uiDC.Error = &Com_Error;
	uiInfo.uiDC.Print = &Com_Printf;
	uiInfo.uiDC.Pause = &UI_Pause;
	uiInfo.uiDC.ownerDrawWidth = &UI_OwnerDrawWidth;
	uiInfo.uiDC.registerSound = engine->trap_S_RegisterSound;
	uiInfo.uiDC.startBackgroundTrack = engine->trap_S_StartBackgroundTrack;
	uiInfo.uiDC.stopBackgroundTrack = engine->trap_S_StopBackgroundTrack;
...
//	uiInfo.uiDC.cursor	= engine->trap_R_RegisterShaderNoMip( "menu/art/3_cursor3" );
	uiInfo.uiDC.whiteShader = engine->trap_R_RegisterShaderNoMip( "white" );
...
//#ifdef MISSIONPACK			// NERVE - SMF - enabled for multiplayer
	engine->trap_LAN_LoadCachedServers();
...
//#endif	// #ifdef MISSIONPACK
	...
	uiInfo.effectsColor = gamecodetoui[(int)engine->trap_Cvar_VariableValue( "color" ) - 1];
	uiInfo.currentCrosshair = (int)engine->trap_Cvar_VariableValue( "cg_drawCrosshair" );
	engine->trap_Cvar_Set( "ui_mousePitch", ( engine->trap_Cvar_VariableValue( "m_pitch" ) >= 0 ) ? "0" : "1" );

	uiInfo.serverStatus.currentServerCinematic = -1;
	uiInfo.previewMovie = -1;

	if ( engine->trap_Cvar_VariableValue( "ui_WolfFirstRun" ) == 0 ) {
		engine->trap_Cvar_Set( "s_volume", "0.8" );
		engine->trap_Cvar_Set( "s_musicvolume", "0.8" );
		engine->trap_Cvar_Set( "ui_WolfFirstRun", "1" );
	}

	engine->trap_Cvar_Register( NULL, "debug_protocol", "", 0 );
}
...
void _UI_KeyEvent( int key, qboolean down ) {
	...
	//  engine->trap_S_StartLocalSound( s, CHAN_LOCAL_SOUND );
	//}
}
...
void _UI_SetActiveMenu( uiMenuCommand_t menu ) {
	...
	if ( Menu_Count() > 0 ) {
		...
		case UIMENU_MAIN:
			//engine->trap_Cvar_Set( "sv_killserver", "1" );
			engine->trap_Key_SetCatcher( KEYCATCH_UI );
			//engine->trap_S_StartLocalSound( engine->trap_S_RegisterSound("sound/misc/menu_background.wav", qfalse) , CHAN_LOCAL_SOUND );
			//engine->trap_S_StartBackgroundTrack("sound/misc/menu_background.wav", NULL);
			if ( uiInfo.inGameLoad ) {
				UI_LoadNonIngame();
			}
			Menus_CloseAll();
			Menus_ActivateByName( "main" );
			engine->trap_Cvar_VariableStringBuffer( "com_errorMessage", buf, sizeof( buf ) );
			if ( strlen( buf ) ) {
#ifdef MISSIONPACK
			...
			engine->trap_S_FadeAllSound( 1.0f, 1000 );    // make sure sound fades up

			// ensure savegames are loadable
			engine->trap_Cvar_Set( "g_reloading", "0" );
...
//----(SA)	added
		case UIMENU_ENDGAME:
			// ensure sound is there for the menu
			engine->trap_S_FadeAllSound( 1.0f, 1000 );
			// ensure savegames are loadable
			engine->trap_Cvar_Set( "g_reloading", "0" );
...
static void UI_DisplayDownloadInfo( const char *downloadName, float centerPoint, float yStart, int font, float scale ) {
	...
	downloadSize = engine->trap_Cvar_VariableValue( "cl_downloadSize" );
	downloadCount = engine->trap_Cvar_VariableValue( "cl_downloadCount" );
	downloadTime = engine->trap_Cvar_VariableValue( "cl_downloadTime" );
...
void UI_DrawConnectScreen( qboolean overlay ) {
	...
	engine->trap_GetClientState( &cstate );

	info[0] = '\0';
	if ( engine->trap_GetConfigString( CS_SERVERINFO, info, sizeof( info ) ) ) {
...
//		Text_PaintCenter(centerPoint, yStart + 48, UI_FONT_DEFAULT, scale, colorWhite, va("Get Psyched!"), ITEM_TEXTSTYLE_SHADOWEDMORE);
	...
	case CA_CONNECTED: {
		...
		engine->trap_Cvar_VariableStringBuffer( "cl_downloadName", downloadName, sizeof( downloadName ) );
		if ( *downloadName ) {
...
void UI_RegisterCvars( void ) {
	...
	for ( i = 0, cv = cvarTable ; i < cvarTableSize ; i++, cv++ ) {
		engine->trap_Cvar_Register( cv->vmCvar, cv->cvarName, cv->defaultString, cv->cvarFlags );
	}
}
...
static void UI_StopServerRefresh( void ) {
	...
	if ( count - uiInfo.serverStatus.numDisplayServers > 0 ) {
		Com_Printf( "%d servers not listed due to packet loss or pings higher than %d\n",
					...
					(int) engine->trap_Cvar_VariableValue( "cl_maxPing" ) );
	}
...
static void UI_DoServerRefresh( void ) {
	...
	if ( ui_netSource.integer != AS_FAVORITES ) {
		if ( ui_netSource.integer == AS_LOCAL ) {
			if ( !engine->trap_LAN_GetServerCount( ui_netSource.integer ) ) {
				wait = qtrue;
			}
		} else {
			if ( engine->trap_LAN_GetServerCount( ui_netSource.integer ) < 0 ) {
				wait = qtrue;
			}
		}
	}
	...
	if ( engine->trap_LAN_UpdateVisiblePings( ui_netSource.integer ) ) {
		uiInfo.serverStatus.refreshtime = uiInfo.uiDC.realTime + 1000;
	} else if ( !wait ) {
...
static void UI_StartServerRefresh( qboolean full ) {
	...
	qtime_t q;
	engine->trap_RealTime( &q );
	engine->trap_Cvar_Set( va( "ui_lastServerRefresh_%i", ui_netSource.integer ), va( "%s-%i, %i at %i:%i", MonthAbbrev[q.tm_mon],q.tm_mday, 1900 + q.tm_year,q.tm_hour,q.tm_min ) );
	...
	//
	if ( ui_netSource.integer == AS_LOCAL ) {
		engine->trap_Cmd_ExecuteText( EXEC_NOW, "localservers\n" );
		uiInfo.serverStatus.refreshtime = uiInfo.uiDC.realTime + 1000;
		return;
	}
	...
	if ( ui_netSource.integer == AS_GLOBAL || ui_netSource.integer == AS_MPLAYER ) {
		...
		if ( strlen( ptr ) ) {
			engine->trap_Cmd_ExecuteText( EXEC_NOW, va( "globalservers %d %s full empty\n", i, ptr ) );
		} else {
			engine->trap_Cmd_ExecuteText( EXEC_NOW, va( "globalservers %d %d full empty\n", i, (int)engine->trap_Cvar_VariableValue( "protocol" ) ) );
		}
	}
}
// -NERVE - SMF
